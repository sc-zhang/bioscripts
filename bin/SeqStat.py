...
def read_fasta(in_file):
	...
	fin.close()
	return sorted(seq_len, reverse=True)
...
def seq_stat(in_file, out_stat):
	file_type = check_file_type(in_file)
	if file_type == 'fa':
		seq_len = read_fasta(in_file)
	elif file_type == 'fq':
		seq_len = read_fastq(in_file)
	elif file_type == 'bam':
		seq_len = read_bam(in_file)
	else:
...
if __name__ == "__main__":
	if len(sys.argv) < 2:
		print("Usage: python %s <in_file> [out_stat]"%sys.argv[0])
	else:
		if len(sys.argv) == 2:
			in_file = sys.argv[1]
			out_stat = ""
		else:
			in_file, out_stat = sys.argv[1:]
		seq_stat(in_file, out_stat)
