...
download_plant_path <- function(db_file) {
  ...
  write.table(plant_pathways_total, file = db_file, quote = F, row.names = F, col.names = F)
}

load_plant_path <- function(db_file) {
  ### Load plant pathway
  plant_pathways_total <- read.table(db_file, sep = "", header = F)
...
generate_go_db <- function(annotation_file, db_path, genus, species, tax_id) {
  ...
  egg <- read.table(annotation_file, header = T, sep = "\t", quote = "")
  egg[egg == ""] <- NA
...
update_kegg_db <- function(db_path, kegg_db_file) {
...
load_kegg_db <- function(kegg_db_file, gene2ko) {
  load(file = kegg_db_file)
  gene2pathway <- gene2ko %>%
    left_join(ko2pathway, by = "Ko", relationship = "many-to-many") %>%
    dplyr::select(GID, Pathway) %>%
    na.omit()
  kegg_db <- list(pathway2name = pathway2name, gene2pathway = gene2pathway)
  return(kegg_db)
}
...
run_go_kegg <- function(db_path, gene2pathway, pathway2name, gene_file, 
  ...
  gene <- read.table(gene_file, header = F)
  gene_list <- gene[, 1]
  GO <- enrichGO(
    gene = gene_list,
  ...
  write.table(GO_df, file = "GO.results.txt", quote = F)
  ...
  write.table(KEGG_df, file = "KEGG.results.txt", quote = F)

  cat("Saving KEGG barplot\n")
  pdf(file = "KEGG_barplot.pdf", width = 15, height = 20)
  print(barplot(KEGG, drop = TRUE, showCateKEGGry = 10))
  dev.off()
...
main_pipe <- function() {
  ...
  if ("input" %in% opt_names && "anno" %in% opt_names && "db" %in% opt_names) {
    ...
    if (!file.exists(kegg_db_file) || "update" %in% opt_names) {
      ...
      update_kegg_db(db_path, kegg_db_file)
    }
    ...
    run_go_kegg(db_path, kegg_db$gene2pathway, kegg_db$pathway2name, gene_file,
                pvalue_cutoff, qvalue_cutoff, padjust_method, ontology)
